...
if __name__ == "__main__":
    ...
    run_path = f"model_collaboration/moco_distill/logs/{task}_{len(model_names)}_{method_name}_multi_student_kd/"
    ...
    for round_idx in range(rounds+1):
        ...
        if round_idx > 0:
            ...
            # mix in an additional 50% from the model itself, 50% from the best student
            single_scores = []
            single_sft_examples = []
            for i in range(len(model_names)):
                with open(os.path.join(run_path, "single_" + str(round_idx-1) + "_" + str(i) + ".json"), "r") as f:
                    data = json.load(f)
                single_scores.append(data["avg_test_score"])
                this_single_sft_examples = []
                for item in data["logs"]:
                    this_single_sft_examples.append({
                        "prompt": item["input"],
                        "completion": item["output"]
                    })
                random.shuffle(this_single_sft_examples)
                single_sft_examples.append(this_single_sft_examples)

            best_single_model_idx = single_scores.index(max(single_scores))
            for i in range(len(model_names)):
                new_sft_examples = sft_examples.copy()
                # add 50% from itself
                num_self_examples = len(single_sft_examples[i]) // 2
                new_sft_examples.extend(single_sft_examples[i][:num_self_examples])
                # add 50% from the best student, it could be that the best student is itself
                num_best_examples = len(single_sft_examples[best_single_model_idx]) // 2
                new_sft_examples.extend(single_sft_examples[best_single_model_idx][:num_best_examples])
                # write to file
                with open(os.path.join(run_path, f"sft_data_model_{i}.jsonl"), "w") as f:
                    for example in new_sft_examples:
                        f.write(json.dumps(example) + "\n")

            # SFT all models on it
            distributed_sft.distributed_sft(
                model_names,
                [os.path.join(run_path, f"sft_data_model_{i}.jsonl") for i in range(len(model_names))],
                gpu_ids,
